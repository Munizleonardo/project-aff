...
function FloatingOfferPanel() {
  ...
  return (
    <div className="hero-offers-panel relative mx-auto h-[340px] w-full max-w-[596px] overflow-hidden rounded-[18px] border border-slate-700/70 bg-[#07101f]/90 p-2 shadow-2xl shadow-blue-950/40 sm:h-[440px] sm:rounded-[24px] sm:p-3">
              ...
              <div key={group} className="flex flex-col gap-3 pb-3">
                {column.products.map((product) => (
                  <Link
                    ...
                    className="hero-offer-card flex h-20 shrink-0 items-center gap-2 rounded-2xl border border-slate-700/70 bg-[#081223] p-2 shadow-lg shadow-black/20 transition hover:border-cyan-400/60 sm:h-[92px] sm:gap-3 sm:p-3"
                  >
                    <img src={product.image} alt={product.name} className="size-12 rounded-xl object-cover sm:size-16" />
...
export function HeroSection() {
  return (
    <section className="hero-section relative overflow-hidden bg-[#030711]">
      ...
      <div className="relative mx-auto flex max-w-7xl flex-col gap-10 px-4 pb-10 pt-14 sm:px-6 md:gap-12 md:px-4 md:pb-20 md:pt-24">
        <div className="grid items-center gap-10 lg:grid-cols-[1fr_0.96fr]">
          <div className="hero-copy flex flex-col gap-6">
            ...
            <h1 className="hero-title max-w-3xl text-4xl font-black leading-[1.12] tracking-tight text-white sm:text-5xl md:text-6xl">
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              <Button asChild className="h-12 rounded-2xl bg-[#38aefb] px-6 text-base font-black text-slate-950 hover:bg-cyan-300">
                <Link href="/ofertas">Ver Ofertas <ArrowRight className="size-5" /></Link>
              </Button>
              <Button asChild variant="outline" className="hero-secondary-button h-12 rounded-2xl border-slate-700 bg-transparent px-6 text-base font-black text-white hover:bg-white/5">
